...
/**
 * Represents the 3D cuboidal area of the world in which objects will cast shadows (basically represents the orthographic
 * projection area for the shadow render pass). It is updated each frame to optimise the area, making it as small as
 * possible (to allow for optimal shadow map resolution) while not being too small to avoid objects not having shadows
 * when they should. Everything inside the cuboidal area represented by this object will be rendered to the shadow map
 * in the shadow render pass. Everything outside the area won't be.
 */
public class ShadowBox {
	...
	/**
	 * Creates a new shadow box and calculates some initial values relating to the camera's view frustum, namely
     * the width and height of the near plane and (possibly adjusted) far plane.
	 * @param lightViewMatrix - Basically the "view matrix" of the light. Can be used to transform a point from
     *                        world space into "light" space (i.e. changes a point's coordinates from being in relation
     *                        to the world's axis to being in terms of the light's local axis).
	...
	/**
	 * Updates the bounds of the shadow box based on the light direction and the camera's view frustum, to make sure
     * that the box covers the smallest area possible while still ensuring that everything inside the camera's view
	...
	/**
	 * Calculates the center of the "view cuboid" in light space first, and then converts this to world space using
     * the inverse light's view matrix.
	...
	/**
	 * Calculates the position of the vertex at each corner of the view frustum in light space (8 vertices in total,
     * so this returns 8 positions).
	 * 
	...
	private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector, Vector3f centerNear, Vector3f centerFar) {
		...
		Vector3f farTop = Vector3f.add(centerFar, new Vector3f(upVector.getX() * farHeight, upVector.getY() * farHeight, upVector.getZ() * farHeight), null);
		Vector3f farBottom = Vector3f.add(centerFar, new Vector3f(downVector.getX() * farHeight, downVector.getY() * farHeight, downVector.getZ() * farHeight), null);
		Vector3f nearTop = Vector3f.add(centerNear, new Vector3f(upVector.getX() * nearHeight, upVector.getY() * nearHeight, upVector.getZ() * nearHeight), null);
		Vector3f nearBottom = Vector3f.add(centerNear, new Vector3f(downVector.getX() * nearHeight, downVector.getY() * nearHeight, downVector.getZ() * nearHeight), null);
	...
	private Vector4f calculateLightSpaceFrustumCorner(Vector3f startPoint, Vector3f direction, float width) {
		Vector3f point = Vector3f.add(startPoint, new Vector3f(direction.getX() * width, direction.getY() * width, direction.getZ() * width), null);
	...
	/**
	 * Calculates the width and height of the near and far planes of the camera's view frustum. However, this doesn't
     * have to use the "actual" far plane of the view frustum. It can use a shortened view frustum if desired by bringing
