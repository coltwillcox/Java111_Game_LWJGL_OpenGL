...
public class ShadowMapEntityRenderer {
	...
	/**
	 * Renders entities to the shadow map. Each model is first bound and then all of the entities using that model are
     * rendered to the shadow map.
	...
	/**
	 * Binds a raw model before rendering. Only the attribute 0 is enabled here because that is where the positions are
     * stored in the VAO, and only the positions are required in the vertex shader.
	...
	/**
	 * Prepares an entity to be rendered. The model matrix is created in the usual way and then multiplied with the
     * projection and view matrix (often in the past we've done this in the vertex shader) to create the mvp-matrix.
